...
export class HuddleController {
  ...
  @Get('meetings')
  getMeetings() {
    return this.huddleService.getMeetings();
  }

  /**
   * This returns all roomName -> roomId mappings
   */
  @Get('roommaps')
  getRoomMaps() {
    return this.huddleService.getRoomMaps();
  }
  ...
  getAccessToken(
    ...
    return this.huddleService.getAccessToken(
      auth,
      initiatorUsername,
      initiatorDisplayname,
  ...
  async meetingEndedEvent(
    ...
    if (this.isValid(timestamp, signature, payload)) {
      ...
      if (errors.length > 0) {
        ...
        this.huddleService.meetingEndedEvent(payload).catch((err) => {
